...
export class ShowCase extends BaElement {
	...
	constructor() {
		...
		const { CoordinateService, EnvironmentService, ShareService, UrlService, GeoResourceService }
			= $injector.inject('CoordinateService', 'EnvironmentService', 'ShareService', 'UrlService', 'GeoResourceService');
	...
	createView() {
		...
		const onClick0 = () => {
			...
			//Example for displaying a layer based on a 'internal' georesource
			//Let's create the internal georesource
			const id = 'hezx6';
			...
			const vgr = new VectorGeoResource(id, label, VectorSourceType.KML).setSource(data, 4326);
			//add georesource to cache
			this._geoResourceService.addOrReplace(vgr);
			//add a layer which displays the georesource
