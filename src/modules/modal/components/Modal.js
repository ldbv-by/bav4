					...
					<div class='modal '>
						<div class='modal__title' @click="${hide}">
							<span class="ba-list-item__pre back-icon" >
								<ba-icon id='back_modal' data-test-id .icon='${arrowLeftShort}' .color=${'var(--primary-color)'} .size=${4}  ></ba-icon>                    							 
							</span>	
							<span class='modal__title-text'>${title}</span>
						</div>
						<div class='modal__content'>${content}</div>
						<div class='modal__actions'>
							<ba-button id='close_modal' data-test-id .label=${translate('modal_close_button')} @click=${hide}></ba-button>
						</div>
					</div>
				</div>
