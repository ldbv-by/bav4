	...
	createView(model) {
		...
		if (url || fileSaveUrl) {
			return html`<style>
				...
				<div class="share_content">${this._getUrlContent(model)}</div>`; //						: unsafeHTML(getInfographicShareCopy(translate('share_dialog_infographic_original'), translate('share_dialog_infographic_copy')))}
		}
		...
		return isToggleNeeded
			? html`<div class="toggle">
					<h4>
						...
						${checkedToggle
							? html`<ba-icon .color=${'var(--primary-color)'} .background=${'var(--primary-color)'} .icon=${peopleSvg} .size=${'1'}></ba-icon>`
							: ''}
					</h4>
					<div class="share_copy_toggle">
						<ba-switch id="toggle" .checked=${checkedToggle} @toggle=${onToggle}>
							<span class="share_copy ${checkedToggle ? 'selected' : ''}" slot="before">${translate('share_dialog_link')}</span>
						</ba-switch>
					</div>
				</div>`
			: html.nothing;
