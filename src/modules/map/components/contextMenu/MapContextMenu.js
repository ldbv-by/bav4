	...
	createView() {
		...
		*/
		...
		<div class='context-menu ${sectorClass}' style=${styleMap(style)}>
			<div class='header'>${translate('map_olMap_contextMenu_header')}<ba-icon class='close' icon='${closeIcon}' title=${translate('map_olMap_contextMenu_close_button')} size=20 color='white'} @click=${closeContextMenu}></ba-icon></div>
			${content}
        </div>`;
	}
