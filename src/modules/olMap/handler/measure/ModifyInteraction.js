 ...
 *
 * we can remove this helper-class after successful integration
 * of the announced PR in openlayers
 * #############################################################
 *
...
import { boundingExtent, buffer as bufferExtent, createOrUpdateFromCoordinate as createExtent } from 'ol/extent';
...
import { fromUserCoordinate, fromUserExtent, getUserProjection, toUserCoordinate, toUserExtent } from 'ol/proj';
 ...
 * @api
 */
export class ModifyInteraction extends PointerInteraction {
	/**
		...
		if (vertexFeature) {
			...
			for (let i = 0, ii = segmentDataMatches.length; i < ii; ++i) {
				...
				if (segmentDataMatch.geometry.getType() === 'Circle' && segmentDataMatch.index === CIRCLE_CIRCUMFERENCE_INDEX) {
					const closestVertex = this.closestOnSegmentData(pixelCoordinate, segmentDataMatch, projection);
	...
	handlePointerAtPixel_(pixel, map, coordinate) {
		const pixelCoordinate = coordinate?.slice() || map.getCoordinateFromPixel(pixel);
		...
		if (vertex) {
          ...
          segment vertex. */
					...
					for (let i = 1, ii = nodes.length; i < ii; ++i) {
						...
						if (
							(coordinatesEqual(closestSegment[0], segment[0]) && coordinatesEqual(closestSegment[1], segment[1])) ||
							(coordinatesEqual(closestSegment[0], segment[1]) && coordinatesEqual(closestSegment[1], segment[0]))
						) {
    ...
    improve it if we want to propose the code changes to openlayers.*/
		return this.segmentExtentFunction(segmentData.feature, segmentData.index || 0) || boundingExtent(segmentData.segment);
	}
	...
	updateSegmentIndices_(geometry, index, depth, delta) {
		this.rBush_.forEachInExtent(geometry.getExtent(), function (segmentDataMatch) {
			if (
				...
				(depth === undefined || segmentDataMatch.depth === undefined || equals(segmentDataMatch.depth, depth)) &&
	...
	projectedDistanceToSegmentDataSquared(point, segmentData, viewProjection, viewExtent) {
		...
		const segments = this?.subsegmentsFunction(feature, index, viewExtent) || [segmentData.segment];
		if (!segments.length) {
			return Infinity;
		}
		return segments
