...
/**
 * Measurement related implementation of {@link module:modules/olMap/tooltip/HelpTooltip~tooltipMessageProviderFunction}
 * @function
...
export const provide = (interactionState) => {
	...
	if (interactionState.type === InteractionStateType.MODIFY) {
		...
		if (interactionState.geometryType && interactionState.geometryType === 'LineString') {
			return translate('olMap_handler_measure_modify_linestring_key_for_delete');
		}
		return translate('olMap_handler_measure_modify_key_for_delete');
	}

	if (interactionState.type === InteractionStateType.SELECT) {
		return translate('olMap_handler_measure_select');
	}
