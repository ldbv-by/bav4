...
export class VectorLayerService {
	...
	_vectorSourceForOaf(geoResource, olVectorLayer, olMap) {
		 ...
		 * But in our case we want to make sure that for each resolution the features are reloaded.
		 */
			...
			if (!geoResource.hasFeatures()) {
				...
				geoResource.features.forEach((baFeature) => {
					...
					if (olFeatures.length === 1) {
						...
						olFeatures[0].set(asInternalProperty('styleHint'), baFeature.styleHint ?? null);
						olFeatures[0].set(asInternalProperty('style'), baFeature.style ?? null);
