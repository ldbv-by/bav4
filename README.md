...
- Model–view–viewmodel (MVVM) structuring:
  ...
  - DOM => *view*
  - Custom elements (web components when needed)  =>  *viewmodel*
...
| `npm run test:webkit` | Runs unit and component tests against the Gnome Web Browser (Epiphany), which is based on WebKit. The browsers must be installed locally. A code coverage report can be found under  `./coverage`  |
