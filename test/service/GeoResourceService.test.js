...
import { FALLBACK_GEORESOURCE_ID_ATKIS_SW, FALLBACK_GEORESOURCE_ID_ATKIS, GeoResourceService } from '../../src/services/GeoResourceService';
	...
	describe('init', () => {
		...
		describe('provider cannot fulfill', () => {
			...
			it('loads two fallback geoResources when we are in standalone mode', async () => {
				...
				expect(georesources[0].id).toBe(FALLBACK_GEORESOURCE_ID_ATKIS);
				expect(georesources[0].getAttribution()[0].copyright.label).toBe('Bayerische Vermessungsverwaltung');
				expect(georesources[1].id).toBe(FALLBACK_GEORESOURCE_ID_ATKIS_SW);
