...
describe('Header', () => {
	...
	describe('when initialized', () => {
		...
		it('adds header bar', async () => {
			...
			expect(element.shadowRoot.querySelectorAll('.header')).toHaveLength(1);
			expect(element.shadowRoot.querySelectorAll('.header__routing-button')).toHaveLength(1);
			expect(window.getComputedStyle(element.shadowRoot.querySelector('.header__routing-button')).display).toBe('none');

			expect(element.shadowRoot.querySelectorAll('.header__button-container')).toHaveLength(1);
			expect(element.shadowRoot.querySelector('.header__button-container').children.length).toBe(3);
			expect(element.shadowRoot.querySelector('.header__button-container').children[0].classList.contains('is-active')).toBe(true);
			...
			expect(element.shadowRoot.querySelector('.header__button-container').children[1].classList.contains('is-active')).toBe(false);

			expect(element.shadowRoot.querySelector('.header__button-container').children[2].innerText).toBe('header_tab_misc_button');
			expect(element.shadowRoot.querySelector('.header__button-container').children[2].classList.contains('is-active')).toBe(false);

			expect(element.shadowRoot.querySelectorAll('#inputFocusButton')).toHaveLength(1);
			...
			expect(element.shadowRoot.querySelectorAll('#clear')).toHaveLength(1);
			...
			expect(element.shadowRoot.querySelectorAll('div.header__emblem')).toHaveLength(1);
			expect(element.shadowRoot.querySelectorAll('a.header__emblem')).toHaveLength(0);
		});
		...
		it('contains test-id attributes', async () => {
			...
			expect(element.shadowRoot.querySelector('#topics_button').hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(true);
			expect(element.shadowRoot.querySelector('#maps_button').hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(true);
			expect(element.shadowRoot.querySelector('#misc_button').hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(true);
			expect(element.shadowRoot.querySelector('#input').hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(true);
		});
		...
		it('renders for standalone', async () => {
			...
			expect(element.shadowRoot.querySelectorAll('div.header__emblem')).toHaveLength(0);
			expect(element.shadowRoot.querySelectorAll('a.header__emblem')).toHaveLength(1);
		...
		it('renders for signIn state', async () => {
			...
			expect(element.shadowRoot.querySelectorAll('.badges-signed-in')).toHaveLength(1);
			expect(element.shadowRoot.querySelectorAll('.badges-signed-in-icon')).toHaveLength(1);
		});
	...
	describe('when close button is swiped', () => {
		...
		it('does NOT hides the header on swiped right, upwards and downwards', async () => {
			...
			expect(element.shadowRoot.querySelectorAll('.is-open .header')).toHaveLength(1);
		});
		...
		it('focused menu-button loses the focus after swipe', async () => {
			...
			mapButton.focus();
			expect(mapButton.matches(':focus')).toBe(true);
			...
			expect(mapButton.matches(':focus')).toBe(false);
		});
	});
	...
	describe('when menu button is clicked', () => {
		...
		it('click button Map', async () => {
			...
			expect(element.shadowRoot.querySelector('.header__button-container').children[0].classList.contains('is-active')).toBe(false);
			expect(element.shadowRoot.querySelector('.header__button-container').children[1].classList.contains('is-active')).toBe(true);
			expect(element.shadowRoot.querySelector('.header__button-container').children[2].classList.contains('is-active')).toBe(false);
		});
		...
		it('click button More', async () => {
			...
			expect(element.shadowRoot.querySelector('.header__button-container').children[0].classList.contains('is-active')).toBe(false);
			expect(element.shadowRoot.querySelector('.header__button-container').children[1].classList.contains('is-active')).toBe(false);
			expect(element.shadowRoot.querySelector('.header__button-container').children[2].classList.contains('is-active')).toBe(true);
		});
	...
	describe('input for search queries', () => {
		describe('when input changes', () => {
			...
			it('shows and hides a clear button', async () => {
				...
				expect(searchContainerElement.classList.contains('is-clear-visible')).toBe(true);
				expect(window.getComputedStyle(clearButton).display).toBe('flex');

				inputElement.value = '';
				inputElement.dispatchEvent(new Event('input'));

				expect(searchContainerElement.classList.contains('is-clear-visible')).toBe(false);
			...
			it('prevents the cursor to flip around the query term on ArrowUp', async () => {
				...
				const arrowUpSpy = vi.spyOn(arrowUpEvent, 'preventDefault');
				const arrowDownSpy = vi.spyOn(arrowDownEvent, 'preventDefault');
		...
		describe('when input clear button is clicked', () => {
			it('updates the store', async () => {
				...
				expect(element.shadowRoot.querySelector('#input').matches(':focus')).toBe(true);
			});
		});
		...
		describe('when input is focused or blurred ', () => {
			...
			describe('in portrait mode', () => {
				...
				it('hides/shows the mobile header', async () => {
					...
					vi.advanceTimersByTime(Header.TIME_INTERVAL_MS + 100);
			...
			describe('in portrait mode and min-width < 80em', () => {
				...
				it('hide mobile header and show again', async () => {
					...
					expect(header.classList.contains('fadein')).toBe(false);
					...
					vi.advanceTimersByTime(Header.TIME_INTERVAL_MS);
					/**
					 ...
					 */
					expect(header.classList.contains('hide')).toBe(false);
					expect(header.classList.contains('fadein')).toBe(true);
				});
			});
			...
			describe('in landscape mode and min-width < 80em', () => {
				...
				it('hide mobile header and show again', async () => {
					...
					expect(logo.classList.contains('fadein')).toBe(false);

					element.shadowRoot.querySelector('#input').focus();

					expect(header.classList.contains('hide')).toBe(true);
					expect(header.classList.contains('fadein')).toBe(false);
					expect(logo.classList.contains('hide')).toBe(true);
					expect(logo.classList.contains('fadein')).toBe(false);
					...
					expect(header.classList.contains('fadein')).toBe(true);
					expect(logo.classList.contains('hide')).toBe(false);
					expect(logo.classList.contains('fadein')).toBe(true);
				});
			});
		});
	});
	...
	describe('when mainMenu.focusSearchField property changes', () => {
		it('sets the focus on the input field', async () => {
			...
			expect(inputEl.matches(':focus')).toBe(false);
	...
	describe('when auth state change', () => {
		it('updates the authButton Button', async () => {
			...
			expect(element.shadowRoot.querySelector('.header__logo-badge').innerText).toBe('header_logo_badge');
			expect(element.shadowRoot.querySelectorAll('.badges-signed-in')).toHaveLength(0);
			expect(element.shadowRoot.querySelectorAll('.badges-signed-in-icon')).toHaveLength(0);
	...
	describe('when orientation changes', () => {
		it("adds or removes 'data-register-for-viewport-calc' attribute", async () => {
			...
			expect(element.shadowRoot.querySelector('.header').hasAttribute(REGISTER_FOR_VIEWPORT_CALCULATION_ATTRIBUTE_NAME)).toBe(false);

			setIsPortrait(true);

			expect(element.shadowRoot.querySelectorAll(`[${REGISTER_FOR_VIEWPORT_CALCULATION_ATTRIBUTE_NAME}]`)).toHaveLength(1);
			expect(element.shadowRoot.querySelector('.header').hasAttribute(REGISTER_FOR_VIEWPORT_CALCULATION_ATTRIBUTE_NAME)).toBe(true);
