		...
		describe('_vectorSourceForData', () => {
			...
			describe('VectorGeoresource has no label', () => {
				it('updates the label of an internal VectorGeoresource and calls the propertyChanged action', async () => {
					...
					const getMetaDataSpy = spyOn(instanceUnderTest, '_getMetaData').and.returnValue({ label, description });
					...
					expect(getMetaDataSpy).toHaveBeenCalledWith(VectorSourceType.KML, sourceAsString, jasmine.any(Array), jasmine.any(KML));
				});
			});
