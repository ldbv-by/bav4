	...
	describe('getBvvOafLoadFunction', () => {
		...
		describe('Number stats are available', () => {
			it('adds the features to the source and updates the `state` property of the layer and the `fetching` property of the network s-o-s', async () => {
				...
				const geoResource = new OafGeoResource('id', 'label', 'https://url.de/', 'collectionId', 4326, 'http://www.opengis.net/def/crs/EPSG/0/CRS84');
			...
			it('updates the `state` and the `props` property of the corresponding layers', async () => {
				...
				const resolution = 42.42;
				const projection = new Projection({ code: 'EPSG:3857' });
				const geoResource = new OafGeoResource('id', 'label', 'https://url.de/', 'collectionId', 3857, 'http://www.opengis.net/def/crs/EPSG/0/3857');
				spyOn(geoResourceService, 'byId').and.returnValue(geoResource);
				const oafLoadFunction = getBvvOafLoadFunction(geoResourceId, olLayer)./*Usually done by the ol.source */ bind(olSource);
		...
		it('includes the `limit` query parameter from a GeoResource', async () => {
			...
			const successCbSpy = jasmine.createSpy();
			const failureCbSpy = jasmine.createSpy();
			const geoResource = new OafGeoResource('id', 'label', 'https://url.de', 'collectionId', 3857, "http://www.opengis.net/def/crs/EPSG/0/3857").setLimit(10);
			spyOn(geoResourceService, 'byId').and.returnValue(geoResource);
			spyOn(httpService, 'get')
				.withArgs(
					expectedUrl,
		...
		it('includes the `filter` query parameter of the OafGeoResource', async () => {
			...
			const successCbSpy = jasmine.createSpy();
			const failureCbSpy = jasmine.createSpy();
			const geoResource = new OafGeoResource('id', 'label', 'https://url.de', 'collectionId', 3857, "http://www.opengis.net/def/crs/EPSG/0/3857").setFilter(filterExpr);
			spyOn(geoResourceService, 'byId').and.returnValue(geoResource);
			spyOn(httpService, 'get')
				.withArgs(
					expectedUrl,
		...
		it('ensures that the `filter` expr. of the olLayer overwrites the `filter` expr. of a OafGeoResource', async () => {
			...
			const successCbSpy = jasmine.createSpy();
			const failureCbSpy = jasmine.createSpy();
			const geoResource = new OafGeoResource('id', 'label', 'https://url.de', 'collectionId', 3857, "http://www.opengis.net/def/crs/EPSG/0/3857").setFilter('filterExprFromGeoResource');
			spyOn(geoResourceService, 'byId').and.returnValue(geoResource);
			spyOn(httpService, 'get')
				.withArgs(
					expectedUrl,
