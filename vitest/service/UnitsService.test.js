...
describe('UnitsService', () => {
	...
	it('provides formatted distance for metric system as default', () => {
		...
		expect(instanceUnderTest.formatDistance(42, 0)).toEqual(expect.objectContaining({ unit: 'm' }));
	});

	it('provides formatted area for bvv metric system as default', () => {
		const instanceUnderTest = new UnitsService();

		expect(instanceUnderTest.formatArea(42, 0)).toEqual(expect.objectContaining({ unit: 'm²' }));
	});

	it('provides formatted angle for bvv metric system as default', () => {
		const instanceUnderTest = new UnitsService();

		expect(instanceUnderTest.formatAngle(42, 0)).toEqual(expect.objectContaining({ unit: '°' }));
	});
	...
	it('replaces null or undefined values with 0 before calling UnitsProvider', () => {
		...
		expect(instanceUnderTest.formatDistance(null, 0)).toEqual(expect.objectContaining({ value: 0 }));
		expect(instanceUnderTest.formatArea(null, 0)).toEqual(expect.objectContaining({ value: 0 }));
		expect(instanceUnderTest.formatAngle(null, 0)).toEqual(expect.objectContaining({ value: 0 }));

		expect(instanceUnderTest.formatDistance(undefined, 0)).toEqual(expect.objectContaining({ value: 0 }));
		expect(instanceUnderTest.formatArea(undefined, 0)).toEqual(expect.objectContaining({ value: 0 }));
		expect(instanceUnderTest.formatAngle(undefined, 0)).toEqual(expect.objectContaining({ value: 0 }));
	});
});
