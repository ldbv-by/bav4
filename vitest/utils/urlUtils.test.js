import { appendQueryParams, getOrigin, getOriginAndPathname, getPathParams, parseBoolean, queryParamsToString, setQueryParams } from '../../src/utils/urlUtils';
...
describe('urlUtils', () => {
    ...
    describe('queryParamsToString', () => {
        it('builds a query string from query parameters', () => {
            expect(queryParamsToString({ one: 42, two: '(((plz+=+12345)))', three: ['a', 'b'] })).toBe(`one=42&two=${encodeURIComponent('(((plz+=+12345)))')}&three=${encodeURIComponent('a,b')}`);
        });
    });
