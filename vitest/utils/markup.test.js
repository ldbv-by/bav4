...
import { BA_FORM_ELEMENT_VISITED_CLASS, decodeHtmlEntities, findAllBySelector, findClosest, forEachBySelector, IFRAME_ENCODED_STATE, IFRAME_GEOMETRY_REFERENCE_ID, LOG_LIFECYLE_ATTRIBUTE_NAME, REGISTER_FOR_VIEWPORT_CALCULATION_ATTRIBUTE_NAME, TEST_ID_ATTRIBUTE_NAME } from '../../src/utils/markup';
...
describe('markup utils', () => {
    ...
    describe('constants', () => {
        ...
        describe('generateTestIds for MvuElements', () => {
            it('provides the correct test id for MvuElements', async () => {
                ...
                expect(divElements).toHaveLength(6);
                ...
                expect(warnSpy).toHaveBeenCalledWith('No data-test-id qualifier found for: mvu-element-parent-0 -> div. Please add either an id or a class attribute.');
                expect(divElements.item(3).hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(false);
                expect(divElements.item(4).hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(false);
                expect(divElements.item(5).hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(false);
                expect(mvuChildElements).toHaveLength(3);
                ...
                expect(mvuChildElements.item(2).hasAttribute(TEST_ID_ATTRIBUTE_NAME)).toBe(false);
            });
    ...
    describe('forEachBySelector', () => {
        ...
        it('applies a function on all elements containing a specific attribute', async () => {
            ...
            expect(callbackSpy).toHaveBeenCalledWith(expect.any(HTMLElement));
        });
    });
